...
export const Level = makeSprite({
  ...
  loop({ props, state, device }) {
    ...
    let { birdGravity, birdY, pipes, score,highScore, birdRotation, birdRotationSpeed} = state;
    ...
    if (didHitPipe(birdY, device.size, pipes)) {
      if(birdRotation > -360){
        birdRotation = birdRotation - birdRotationSpeed;
      }
      else{
        console.log(birdY);
        birdRotation = 0;
  ...
  render({ state, device }) {
    const { size } = device;
    console.log(`bird:${Math.max(-30, state.birdGravity * 3 - 30)},${state.birdRotation}`)
    return [
      t.rectangle({
        color: "#add8e6",
      ...
      Bird({
        ...
        x: birdX,
        y: state.birdY, 
        rotation: state.birdRotation || Math.max(-30, state.birdGravity * 3 - 30),
      }),
